...
electrónico que la anima, continuamos en el Cap. \ref{ch:progPLC} con la
...
\gls{scada} mediante P-CIM para Windows (AFCON), con el objetivo de poder
...
proyecto.
El lector podrá entender, y por lo
...
mediante el \gls{scada}, que será detallado en la sección
\todo{agregar la
sección donde se describe la operación mediante el scada. Anexo?}
...
problemas.\todo{referencia?}

Es entonces claro que la utilización de un sistema \gls{scada} reduce 
las tareas asociadas al control y monitoreo de plantas industriales.
...
(\url{www.afcon-inc.com}). El software, provee un ambiente tanto de desarrollo
como de ejecución de aplicaciones \gls{scada}\footnote{Este software puede
...
estructura en capas del programa, como se describió en la Sec.
...
comunicación con el \gls{plc} que controla la planta, por medio de algún bus.
Es decir, lograr un buen funcionamiento de la capa de comunicación. En nuestro
caso
...
bus de comunicaciones\footnote{El driver Modbus se puede descargarse desde
...
La base de datos esta formada por elementos, llamados bloques, que son
utilizados para:
\begin{itemize}
...
16 bits) y alarmas (en grupos de 16 bits).
  \item \textbf{Tiempo de Escrutinio (scan) y de Fase:} que son
respectivamente:
    \begin{itemize}
    ...
    \end{itemize}
    Ambos valores se encuentran en el rango de 1 a 255 unidades, donde la
unidad de tiempo por defecto es un segundo.
...
La configuración de los bloques debe ser tal que para:
\begin{itemize}
...
archivo asignándole un nombre.
   ...
   \hline
	...
	movidos, redimensionados y editados. Además, pueden
asignarse propiedades.
	&\begin{center}
	  %\rule{0.4\textwidth}{0.3\textwidth}
	   \includegraphics[width=0.4\textwidth]
...
Se muestra en la Fig. \ref{fig:hmiscada} el resultado de la \gls{hmi}
diseñada.
...
\section{Ejecución}
\label{sec:Ejecucion}
Una vez que han sido configuradas cada una de las partes podemos utilizar
