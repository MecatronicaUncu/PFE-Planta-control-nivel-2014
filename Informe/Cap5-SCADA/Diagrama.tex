...
{\texttt{EMUL\_TD\_WRITE}};
  ...
  \node[annot,above of=40001, node distance=1cm] (modbus) {MODBUS};
  \node[annot,above of=io, node distance=1cm] (aio) {Analog Input Ouput};
  \node[annot,right of=aio, node distance=0.5*5cm] (calc) {Calculation};
  \node[annot,above of=eio, node distance=1cm] (emul) {Emulation};
  \node[annot,above of=a1, node distance=1cm] (emula) {Emulation Adress};
