...
Es uno de los objetivos principales para validar el correcto funcionamiento de
la planta.

En el presente capítulo se describe tanto el algoritmo del programa de
control de la planta. como así también el
lenguaje utilizado para implementar este algoritmo.
...
se pensó en el algoritmo para cumplir con todas las tareas que el controlador
debía realizar (gestión de alarmas, adquisición de parámetros, control de la
válvula, encendido y apagado de motores).
Se observa en en el algoritmo \ref{img:algoritmo}, como el controlador
sistema debe comportarse, de manera estructurada, para cumplir estas tareas.
...
\subsection{Breve descripción de Ladder}
...
salida.
De esta manera, vamos describiendo paso a paso las acciones que deseamos que
...
Los elementos de entrada principales en Ladder son:
 ...
 \begin{itemize}
  ...
  \item Función lógica (comparación de una variable con una constante)
 \end{itemize}

Los elementos de entrada se combinan mediante acciones lógicas \verb|AND|,
\verb|OR|, \verb|NOT| (representadas mediante ``conductores'') y otros bloques
de funciones (timers, contadores, registros).
Se obtienen así nuevas señales, que pueden ser utilizadas para realizar
acciones de salida.

Las acciones de salida se representan como bobinas. Encontramos los
siguientes tipos:
...
entrada se hace inactiva, el valor de salida se mantiene.
...
\todo{Anexo de lo que imprimimos desde el twido soft}Lo primero que se presenta
son la acción realizada en cada RUNG (pasos del programa).
